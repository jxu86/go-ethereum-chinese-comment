...
type TxPoolConfig struct {
	...
	PriceLimit uint64 // Minimum gas price to enforce for acceptance into the pool 价格超出比例，若想覆盖一笔交易的时候，若价格上涨比例达不到要求，那么不能覆盖
	PriceBump  uint64 // Minimum price bump percentage to replace an already existing transaction (nonce) 替换现有交易的最低价格涨幅百分比（一次）

	AccountSlots uint64 // Number of executable transaction slots guaranteed per account 每个账户的可执行交易限制
	GlobalSlots  uint64 // Maximum number of executable transaction slots for all accounts 全部账户最大可执行交易
	AccountQueue uint64 // Maximum number of non-executable transaction slots permitted per account 单个账户不可执行的交易限制
	GlobalQueue  uint64 // Maximum number of non-executable transaction slots for all accounts 全部账户最大非执行交易限制

	Lifetime time.Duration // Maximum amount of time non-executable transaction are queued 一个账户在queue中的交易可以存活的时间
}
...
type TxPool struct {
	...
	currentState  *state.StateDB // Current state in the blockchain head 当前头区块对应的状态
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	pool.locals = newAccountSet(pool.signer) // 初始化本地账户
...
// 获取交易池pending的交易，返回账户地址对于的Transaction数组，账户的交易按nonce排序
